...
//                 // To Customer
//                 await sendEmail({
//                     email: req.user?.email || orderData.email, // Ensure email is passed or in req.user
...
//                     message: `<p>A new online payment was received from ${req.user?.name || 'Customer'}.</p><p>Amount: ₹${orderData.totalPrice}</p>`
//                 });
...
// 2. Verify Payment AND Save to Database + Notify Admin
exports.verifyAndSaveOrder = async (req, res) => {
    try {
        const { 
            razorpay_order_id, 
        ...
        if (savedOrder) {
            ...
            try {
                ...
                console.log("⚠️ Bell Notification failed (Online Order):", notifyError.message);
            }
        ...
        // Return the response so the frontend knows to finish and refresh the UI
