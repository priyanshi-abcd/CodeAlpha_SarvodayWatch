...
//   return (
//     <>
//       {/* IMPORTANT: This Navbar is fixed. 
//         In your App.js or page components, wrap your content in a div with 'pt-20' 
//         to prevent the content from hiding behind the navbar.
//       */}
...
//             <div className="hidden sm:flex items-center space-x-6 border-r border-gray-200 pr-6 mr-2">
//               {userData ? (
...
//                   {cartCount > 0 && (
//                     <span className="absolute -top-2 -right-2 bg-[#D4AF37] text-white text-[8px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
//                       {cartCount}
//                     </span>
//                   )}
//                 </Link>
...
//          This only affects the page content immediately following the navbar.
//       */}
...
// Inside Navbar.jsx, update your useEffect block to this:
...
const Navbar = () => {
  const navigate = useNavigate();
  const location = useLocation(); // 🔄 Instantiated to listen to page route changes
  ...
  // 🔥 CORE SYSTEM SYNC: Overrides the context variable to match your exact database states
  const accurateUnreadCount = notifications.filter(n => n.isRead === true).length;

  // 🔄 LIVE ROUTE WATCHER: Refetches fresh data when transitioning between Home and Admin
  useEffect(() => {
...
const fetchWishlistCount = useCallback(async () => {
    ...
    // 1. Safety check: If no token or admin, count is 0
    if (!token || isAdmin) {
        setWishlistCount(0);
        return;
    }
    ...
    try {
        // 2. Fetch the flat array directly from the API
        const { data } = await axios.get(`http://localhost:5000/api/admin/wishlist?t=${Date.now()}`, {
            headers: { Authorization: `Bearer ${token}` }
        });
        
        // 3. Since data is now a flat array (e.g., [item1, item2, item3]),
        // the count is simply the length of that array.
...
//   const fetchWishlistCount = useCallback(async () => {
//     console.log("Navbar: Received update event, re-fetching count...");
//     const token = userData?.token;
//     if (!token || isAdmin) {
//       setWishlistCount(0);
//       return;
...
//     // FIX: Sum up items across all wishlist entries (or just the first entry)
//     // If your backend returns an array of documents:
//     // const totalItems = data.reduce((acc, entry) => acc + (entry.items?.length || 0), 0);
//     const totalItems = Array.isArray(data) 
//         ? data.reduce((sum, entry) => sum + (entry.items?.length || 0), 0) 
//         : 0;
//     // console.log("Navbar: Calculated total items:", totalItems);
//     localStorage.setItem('wishlistCount', totalItems);
//     // setWishlistCount(totalItems);
//     setWishlistCount(prev => {
//             console.log("Navbar: Changing count from", prev, "to", totalItems);
//             return totalItems;
//         });
//   } catch (err) {
//     console.error("Error fetching wishlist count", err);
...
//     // Standardize to single event names
//     window.addEventListener('wishlistUpdate', fetchWishlistCount);
//     window.addEventListener('cartUpdate', fetchCartCount);
...
// }, [fetchWishlistCount, fetchCartCount]);
  ...
  //     } catch (err) {
  //       console.error("Auto-clear failed:", err.response?.data || err.message);
  //     }
  //   }
  // };

  const handleBellClick = async () => {
    // 1. Check if there are active unread notifications to clear
    if (accurateUnreadCount > 0) {
      try {
        const userInfo = JSON.parse(localStorage.getItem('userInfo'));
        ...
        // 2. Identify the target type based on whether an Admin or Customer clicked it
        const targetType = isAdmin ? 'NEW_ORDER' : 'ORDER_STATUS';

        // 3. Send the custom clear request to your backend route
        await axios.put('http://localhost:5000/api/notifications/mark-read', { type: targetType }, config);
        
        // 4. Instantly refresh the global notification context state array
        fetchNotifications();
      } catch (err) {
    ...
    // 5. Navigate the user cleanly to their respective management route
    navigate(isAdmin ? "/admin" : "/profile");
  };
          ...
          <div className="flex items-center space-x-4 md:space-x-6 text-gray-800">
            ...
            {userData && (
              <div className="relative group">
                ...
                >
                  <Bell size={20} />
                  {/* 🔥 MAKE SURE THIS USES accurateUnreadCount */}
                  {accurateUnreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[8px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                      {accurateUnreadCount}
                    </span>
